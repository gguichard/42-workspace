...
/*   Updated: 2019/02/10 04:43:24 by gguichar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		draw_fractal(t_data *data)
{
	...
	while (x < data->winsize.width)
	{
		...
		{
			re = (x - data->winsize.width / 2.0) * 4.0
				/ data->winsize.width;
			im = (y - data->winsize.height / 2.0) * 4.0
				/ data->winsize.height;
			iters = data->fract_fn(&data->motion, re, im, data->max_iters);
			data->lib.img_data[y * data->winsize.width + x] = iters < data->max_iters ? get_fract_color(iters) : 0;
			y++;
		}
		x++;
	}
