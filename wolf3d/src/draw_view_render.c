/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   draw_view_render.c                                 :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: gguichar <gguichar@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/11/25 13:19:55 by gguichar          #+#    #+#             */
/*   Updated: 2019/11/25 18:55:42 by gguichar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <stdint.h>
#include "wolf3d.h"
#include "tile_inf.h"
#include "ray_inf.h"
#include "vec2.h"
#include "utils.h"

static inline uint32_t	darker_color(uint32_t color, double percent)
{
	uint8_t	a;
	uint8_t	r;
	uint8_t	g;
	uint8_t	b;

	a = (color >> 24) & 0xff;
	r = (color >> 16) & 0xff;
	g = (color >> 8) & 0xff;
	b = color & 0xff;
	r *= (1 - percent);
	g *= (1 - percent);
	b *= (1 - percent);
	return ((a << 24) | (r << 16) | (g << 8) | b);
}

static inline int		get_z_buffer(t_ctx *ctx, t_texture_inf *text_inf
	, uint32_t color)
{
	if (text_inf == &ctx->textures[PORTAL_ENTRY_TEXTURE]
		|| text_inf == &ctx->textures[PORTAL_EXIT_TEXTURE])
	{
		if ((color & ALPHA_CHANNEL) == 0 || (color & 0xffffff) != 0x0)
			return (100);
		return (1);
	}
	return (10);
}

static inline void		plot_pixel(t_ctx *ctx, size_t index, uint32_t color
	, int z_value)
{
	if (ctx->use_z_buffer)
	{
		if (z_value <= ctx->z_buffer[index])
			return ;
		ctx->z_buffer[index] = z_value;
	}
	if ((color & ALPHA_CHANNEL) == ALPHA_CHANNEL)
		ctx->window.pixels[index] = color;
}

void					draw_texture(t_ctx *ctx, t_ray_inf *ray_inf
	, t_texture_inf *text_inf, int x)
{
	t_draw_ctx	draw_ctx;
	t_vec2i		text_coord;
	double		darker;
	int			y_index;
	uint32_t	color;

	setup_draw_ctx(ctx, ray_inf, &draw_ctx);
	text_coord.x = (text_inf->width - 1) * ray_inf->position;
	darker = clamp(ray_inf->length / FOG_DIST, 0, 1);
	y_index = 0;
	while (y_index < draw_ctx.wall_screen_height)
	{
		text_coord.y = (text_inf->height - 1)
			* ((y_index + draw_ctx.wall_top) / (double)draw_ctx.wall_height);
		color = text_inf->pixels[text_coord.y * text_inf->width + text_coord.x];
		plot_pixel(ctx, (y_index + draw_ctx.wall_screen_top)
			* ctx->window.size.width + x, darker_color(color, darker)
			, get_z_buffer(ctx, text_inf, color));
		y_index++;
	}
}

void					draw_column(t_ctx *ctx, t_ray_inf *ray_inf, int x)
{
	int		half_height;
	int		y_index;
	double	darker;

	half_height = ctx->window.size.height / 2;
	y_index = 0;
	while (y_index < half_height)
	{
		darker = y_index / (double)half_height;
		plot_pixel(ctx, y_index
			* ctx->window.size.width + x, darker_color(CEIL_COLOR, darker), 5);
		plot_pixel(ctx, (ctx->window.size.height - y_index - 1)
			* ctx->window.size.width + x, darker_color(CEIL_COLOR, darker), 5);
		y_index++;
	}
	draw_texture(ctx, ray_inf, &ctx->textures[ray_inf->direction], x);
}
