...
/*   Updated: 2019/12/15 16:08:33 by gguichar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	, t_free_alloc *block, int order)
{
	...
	else if (free_level > 0)
	{
		...
		if (region->data[blk_idx].free
			&& region->data[blk_idx].order == order
			&& remove_from_free_list(region->free_list + free_level, buddy))
		{
			blk_idx = get_block_index(region, (block < buddy ? buddy : block));
			((uint64_t *)&region->data[blk_idx])[0] = 0;
			block = (block < buddy ? block : buddy);
			region->data[get_block_index(region, block)].order = ++order;
			return (coalesce_block_with_buddy(region, block, order));
		}
	}
	region->data[get_block_index(region, block)].free = 1;
...
void						free_routine(t_zone *zone, void *ptr)
{
	...
	if (region->data[block_index].order == 0
		|| region->data[block_index].free)
		return ;
	order = region->data[block_index].order;
